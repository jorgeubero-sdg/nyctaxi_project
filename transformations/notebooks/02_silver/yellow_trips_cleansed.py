...
df = spark.read.table("az_core_realstate_internal_training_catalog.01_bronze_jrg.yellow_trips_raw").filter(f"tpep_pickup_datetime >= '{two_months_ago_start}' AND tpep_pickup_datetime < '{one_month_ago_start}'")
...
df.write.mode("append").saveAsTable("az_core_realstate_internal_training_catalog.02_silver_jrg.yellow_trips_cleansed")