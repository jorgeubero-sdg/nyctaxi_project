...
df_trips = spark.read.table("az_core_realstate_internal_training_catalog.02_silver_jrg.yellow_trips_cleansed").filter(f"tpep_pickup_datetime > '{two_months_ago_start}'")

# Load taxi zone lookup data from the Silver layer
df_zones = spark.read.table("az_core_realstate_internal_training_catalog.02_silver_jrg.taxi_zone_lookup")
...
df_join_final.write.mode("append").saveAsTable("az_core_realstate_internal_training_catalog.02_silver_jrg.yellow_trips_enriched")